...
public class EntityBlockBomb extends EntityThrowable {
	...
	public EntityBlockBomb(World world, EntityLivingBase thrower, ItemStack[] items) {
		...
		this.setSize(0.6F, 0.6F);
		this.gravity = 0.1f;
		this.state = ((ItemBlock)items[0].getItem()).block.getStateForPlacement(worldObj, BlockPos.ORIGIN, EnumFacing.UP, 0, 0, 0, items[0].getMetadata(), thrower, EnumHand.MAIN_HAND);
								...
								if (items != null) {
									...
									if (this.worldObj.rand.nextBoolean() &&
										...
										for (int i=0; i<items.length; ++i)
											if (items[i] != null && items[i].func_190916_E() > 0 && items[i].getItem() instanceof ItemBlock &&
											thrower instanceof EntityPlayer && ((EntityPlayer) this.thrower).canPlayerEdit(pos, EnumFacing.UP, items[i])) {
												IBlockState state = ((ItemBlock)items[i].getItem()).block.getStateForPlacement(worldObj, pos, EnumFacing.UP, 0, 0, 0, items[i].getMetadata(), thrower, EnumHand.MAIN_HAND);
												if (((ItemBlock)items[i].getItem()).placeBlockAt(items[i], (EntityPlayer) thrower, worldObj, pos, EnumFacing.UP, 0, 0, 0, state)) {
													itemFound = true;
													items[i].func_190918_g(1);
													break;
												}
											}
											else
